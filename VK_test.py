...
keyboard_work.add_button('Добавить в Избранное',
                         color=VkKeyboardColor.PRIMARY)
keyboard_work.add_openlink_button('Страница пользователя',
...
def bot_work(sender, message):
    authorize.method('messages.send',
                     {'user_id': sender,
                      ...
                      'keyboard': keyboard_work.get_keyboard()})
...
for event in longpoll.listen():
    if event.type == VkEventType.MESSAGE_NEW and event.to_me and event.text:
        ...
        if reseave_message == "Начать":
            ...
            photos.append('photo{}_{}'.format(upload_image['owner_id'],
                                              upload_image['id']))
